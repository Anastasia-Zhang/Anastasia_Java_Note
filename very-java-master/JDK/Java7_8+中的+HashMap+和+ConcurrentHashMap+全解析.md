...
![1](https://clsaa-markdown-imgbed-1252032169.cos.ap-shanghai.myqcloud.com/very-java/2019-04-24-170041.png)
...
1. **判断是不是要扩容 如果为是空数组的话就要初始化数组：**
   1. **计算数组的容量 capacity （保证是2的n次方）**
   2. **计算扩容后的阈值 （capacity * loadFactor）**
   3. **初始化table**
2. **计算hash值，计算数组下标的位置 `hash & （length - 1）`就是取 hash 的低几位**
3. **遍历链表或者数组，看是否存在相同的key，存在相同的key直接替换掉，完成后返回**
4. **不存在相同 key 的话进行 addEntry 操作**
   1. **判断是否要扩容（当前链表否大于阈值，是的话扩容）**
   2. **重新计算下标**
   3. **新 new 一个 Entry，添加到表头**
...
![3](https://clsaa-markdown-imgbed-1252032169.cos.ap-shanghai.myqcloud.com/very-java/2019-04-24-170042.png)
        ...
        if (e != null) {
            ...
            if (next == null)   // 该位置处只有一个元素，那比较好办
                ...
                for (HashEntry<K,V> last = next;
                    ...
                    // 0307 这个位置和他的上一个位置不相等 更新lastRun结点和idx的值
                    if (k != lastIdx) {
                        lastIdx = k;
                        lastRun = last;
                    }
...
![2](https://clsaa-markdown-imgbed-1252032169.cos.ap-shanghai.myqcloud.com/very-java/2019-04-24-170043.png)
...
![4](https://clsaa-markdown-imgbed-1252032169.cos.ap-shanghai.myqcloud.com/very-java/2019-04-24-170044.png)
