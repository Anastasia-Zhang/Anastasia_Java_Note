...
* 使用索引可以大大加快检索速度（索引将无序的数据变成有序的数据）

  > 如果是基于B+树的索引，他的检索取决于树的高度 相关，O(h)=O(logdN)，如果扫描全表可能就是O(N)

* 唯一索引：可以保数据库每一行的数据唯一性
...
* 增加索引的维护开销，当表中的数据进行增加、删除和修改时，索引也需要维护
* 增加索引的空间开销，索引也需要占一定的空间
...
- 索引所在的键值经常修改的情况，会增加索引的维护

- 范围查询时范围基本涵盖全表数目时，此时和扫描全表一样，引入索引会有开销

  检索比率：查询的范围占全表范围的比例 一般为10%，低于此值建立索引

- 不推荐使用业务主键。使用InnoDB时使用业务无关的整型自增主键作为主键，如果使用业务主键的话，会寻找当前最大值并加+ 1 2. 用一个专用表保存”下一个值“且加锁更新。这样但如果插入并发性过高，在主键索引的创建操作上会发生十分严重的资源竞争

- 避免将索引列使用NULL值

- `where f(indexed_column) = 'some value' ` 索引无法命中。无法保证函数的地址与该列索引的顺序相同，若不同，则索引实效还会扫描整张表。
...
* 在经常要搜索的列添加索引
*  where 语句中的列上面添加索引
* 对经常需要排序的列中建立索引，因为索引本身就是有序的，查询时可以利用索引排序降低性能损耗
* 外键使用索引，加快连接的速度
...
* MyISAM ：B+Tree叶节点data 域存放的是数据记录的地址。索引检索的时候，按照 B 树的检索算法搜索索引，如果指定的Key存在，则取出 data 的值，再以该地址取出数据。
* InnoDB：数据文件本身就是索引文件是按照B树组织的索引结构，不像MyISAM，数据文件和索引文件是分开的。
  * 主索引：**叶节点data域保存了完整的数据记录**。这个索引的key就是数据表的主键。InooDB数据表文件本身就是主索引（聚集索引），其余的索引都作为辅助索引。
  * **辅助索引的data域存储了记录主键的值**而不是地址
  * 根据主索引搜索时，直接找到key结点可以取出数据
  * 根据辅助索引搜素时，则需要先取出主键值，在遍历一遍主索引



### 覆盖索引

如果一个索引包含所有需要查询的字段的值，则称之为覆盖索引。有以下优点：
...
![img](E:\研究生学习\Typora图片\45016e98-6879-4709-8569-262b2d6d60b9.png)



辅助索引的叶子节点的 data 域记录着主键的值，因此在使用辅助索引进行查找时，需要先查找到主键值，然后再到主索引中进行查找。

![img](E:\研究生学习\Typora图片\7c349b91-050b-4d72-a7f8-ec86320307ea.png)
...
徐刘根 [Java后端技术](javascript:void(0);) *2018-03-06*
...
4、没有键值相等的节点； 
![img](E:\研究生学习\Work\技术笔记\MySQL索引.assets\640-1585926543772.webp) 
...
一个二叉查找树是由n个节点随机构成，所以，对于某些情况，二叉查找树会退化成一个有n个节点的线性链。如下图： 
 ![img](E:\研究生学习\Work\技术笔记\MySQL索引.assets\640-1585926615286.webp)
...
![img](E:\研究生学习\Work\技术笔记\MySQL索引.assets\640-1585926543798.webp) 
...
![img](E:\研究生学习\Work\技术笔记\MySQL索引.assets\640-1585926543780.webp)
...
![img](E:\研究生学习\Work\技术笔记\MySQL索引.assets\640-1585926674688.webp)
...
![img](E:\研究生学习\Work\技术笔记\MySQL索引.assets\640-1585926696327.webp)
